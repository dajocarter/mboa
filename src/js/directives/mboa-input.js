angular.module('mboa').directive('mboaInput', function(Answers, Hints) {
  function getTemplate(tElt, tAttrs) {
    ...
    var gradedInput = "<input type='text' ng-model='bindedModel' ng-change='gradeAnswer(bindedModel)' class='check-input' /><i ng-show='correct' class='icon ion-ios-checkmark input'></i>";
    var hintInput = "<input type='text' ng-model='bindedModel' ng-change='gradeAnswer(bindedModel)' class='check-input'><i ng-class='{show: !showHint && !correct}' class='icon ion-ios-help-empty input' ng-click='toggleHint()'></i><i ng-class='{show: correct}' class='icon ion-ios-checkmark input'></i><div ng-show='showHint' class='hint'>{{hint}}<i class='icon ion-ios-close' ng-click='toggleHint()'></i></div>"
